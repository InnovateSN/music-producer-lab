...
cd /home/user/music-producer-lab
...
for i in {1..10}; do
  if [ $i -eq 1 ]; then
    ...
    prev_num=$((i-1))
    prev_url="lesson-ear-${prev_num}.html"
  fi
  
  if [ $i -eq 10 ]; then
    next_url="labs.html"
    next_text="Back to Labs &rarr;"
  else
    next_num=$((i+1))
    next_url="lesson-ear-${next_num}.html"
    next_text="Next Lesson &rarr;"
  fi

  cat > "lesson-ear-${i}.html" << EOF
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <title id="mpl-page-title">Lesson ${i} · Ear Training | Music Producer Lab</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover" />
    <meta name="description" id="mpl-meta-description" content="Ear Training Lesson ${i}" />
  ...
  <body>
    ...
    <main class="main-content container">
      ...
      <section style="margin-top: var(--space-2xl); margin-bottom: var(--space-2xl);">
        <div style="display: flex; gap: var(--space-md); justify-content: space-between; flex-wrap: wrap;">
          <a href="${prev_url}" class="btn btn-secondary">&larr; Previous</a>
          <a href="${next_url}" class="btn btn-primary">${next_text}</a>
        </div>
      </section>
    </main>
    ...
    <script type="module">
      import { lessonConfig } from './configs/lesson-ear-${i}.config.js';
...
echo "All 10 Ear Training HTML files created successfully!"
