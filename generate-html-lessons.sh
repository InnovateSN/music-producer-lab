...
# Function to create lesson HTML
create_lesson_html() {
  local module=$1
  local number=$2
  local filename="lesson-${module}-${number}.html"
...
<html lang="en">
  ...
  <body>
    ...
    <main class="main-content container">
      ...
      <!-- SCRIPT IMPORTS -->
      <script type="module">
        import { lessonConfig } from './configs/CONFIGPATH';
...
EOF
  ...
  # Replace config path
  sed -i "s|CONFIGPATH|lesson-${module}-${number}.config.js|g" "$filename"
  echo "Created $filename"
}

# Generate Sound Design HTML (15 lessons)
for i in {1..15}; do
  create_lesson_html "sound-design" "$i"
done

# Generate Mixing HTML (15 lessons)
for i in {1..15}; do
  create_lesson_html "mixing" "$i"
done

# Generate Vocals HTML (12 lessons)
for i in {1..12}; do
  create_lesson_html "vocals" "$i"
done

# Generate Mastering HTML (10 lessons)
for i in {1..10}; do
  create_lesson_html "mastering" "$i"
done

echo ""
echo "✅ Generated all HTML lesson files successfully!"
echo "Total files created: 52"
